...
**§14 revision note (2026-07-26):** the paragraph above ("Stay in your lane...") was added to reconcile a real, previously-unaddressed gap between this section and §10/§19 — this template never referenced either section's refusal rules, discovered live via the Sprint 3 Golden Set run (G-12 never reaching generation; see [Findings Log](findings-log.md) #10). Every other bullet in §19's checklist is already covered elsewhere in this template (grounding covers price/discount invention, the escalation paragraph covers Service-Recovery non-upsell, the voice paragraph covers the bot/human-identity rule) — this new paragraph closes the remaining gap rather than restating what's already there.
