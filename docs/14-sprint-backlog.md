...
**Deferred — not blocked, just not yet built (the admin shell's Supabase Auth is resolved; these are surface work over `IngestionService`):**
...
- `policies.md` → `FAILED` on a genuine Voyage 429 (rate limit) from this session's own heavy testing traffic — external and transient, not a code defect. Re-running `prisma/ingest-bellevue.mjs` (idempotent) once the rate-limit window clears will pick it up.
...
**Remaining blocker — Supabase creds for the admin HTTP surface (unrelated to the above, already resolved separately per Sprint 1's admin-shell section):** `POST /v1/admin/knowledge/documents`, the upload screen, and Supabase Storage are still thin surface work over the now-fully-verified `IngestionService` — no new blocker, just not yet built.
