...
### 11. Off-topic/refusal-category messages can be intercepted by Low Confidence before any refusal instruction ever applies — OPEN

**Found:** Verifying the fix for finding 10 (`apps/api/verify-refusal-content.mjs`).
**Problem:** Fixing `base.md`'s content gap (finding 10) does not guarantee the new refusal instructions ever get a chance to run. Three of six reproduced scenarios (G-12 competitor comparison, item 56 roleplay/jailbreak-for-a-password, item 60 repeated discount pressure) scored Low Confidence on retrieval and were routed to the honest-fallback text *before generation ever ran* — the exact same failure mode finding 10 originally exposed via G-12, now confirmed to still apply to an entire category of message, not just that one scenario.
**Root cause:** These messages are semantically off-topic relative to the hotel's actual indexed content (a competitor's name, a fake roleplay premise, a repeated discount ask don't resemble real hotel-content chunks), so they score genuinely low similarity — the confidence gate is doing exactly what it's designed to do (AI Engine §5: "Low: Do not answer from the model"). The gate has no way to distinguish "we have no knowledge-base answer for this" from "this is a refusal-category message that doesn't need retrieval at all" — both look identical to it: low similarity, no strong chunk match.
**Fix chosen — and why:** Not yet fixed. This is a distinct, separate architectural question from finding 10's content gap — whether/how off-topic-refusal messages should bypass or be exempted from the confidence gate (a new classifier signal? a fixed list of non-retrieval-dependent categories that always reach generation?) touches the deliberate, separate Low-Confidence design (AI Engine §5) and shouldn't be decided as a side effect of a prompt-content fix. Flagged here rather than guessed at.
**Verification:** N/A yet. `apps/api/verify-refusal-content.mjs` already reports the real confidence band/event sequence for each scenario (see its `[G-12 diagnostic]` line) — re-run once a fix is scoped and check G-12/56/60 actually reach generation.

---

**Next entry number: 12.**
